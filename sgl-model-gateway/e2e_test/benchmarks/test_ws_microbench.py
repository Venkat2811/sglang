...
async def _run_single_sample(ws_url: str, model: str) -> dict[str, float | int]:
    import websockets
    ...
    for concurrency in concurrency_levels:
        ...
        for _ in range(samples_per_concurrency):
            batch = await asyncio.gather(
                *[_run_single_sample(ws_url, model) for _ in range(concurrency)]
            )
        ...
        summary = {
            "samples": len(samples),
            "connect_ms_p50": _percentile(values("connect_ms"), 0.50),
            "connect_ms_p95": _percentile(values("connect_ms"), 0.95),
